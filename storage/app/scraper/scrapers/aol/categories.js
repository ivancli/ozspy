...
class Scraper {
    constructor(argvs) {
        ...
        this.url = 'https://www.appliancesonline.com.au/sitemap-categories.xml';
        this.categories = [];
    ...
    parse(html) {
        convert.xmlDataToJSON(html).then(json => {
            if (typeof json.urlset.url === 'object') {
                let urls = json.urlset.url.map(url => url.loc[0]);
                urls.forEach(url => {
    ...
    generateCategory(paths, attr, url) {
        if (paths.length > 0) {
            ...
            if (existingCategories.length > 0) {
                ...
                existingCategory = {};
                existingCategory.name = this.toTitleCase(path.replace('.aspx', '').replace((new RegExp('-', 'g')), ' '));
