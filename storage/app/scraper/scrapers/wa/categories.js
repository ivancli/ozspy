...
class Scraper {
    constructor(argvs) {
        ...
        this.url = 'https://www.winningappliances.com.au/sitemap.xml';
        this.categories = [];
    ...
    parse(html) {
        convert.xmlDataToJSON(html).then(json => {
            if (typeof json.urlset.url === 'object') {
                let urls = json.urlset.url.map(url => url.loc[0]).filter(url => url.indexOf('/c/') > -1);
                urls.forEach(url => {
    ...
    stripUrl(url) {
        return url.substr(url.indexOf('/c/') + 3).split('/').filter(result => result !== "");
    }
