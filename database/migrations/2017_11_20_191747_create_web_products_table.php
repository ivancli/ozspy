...
class CreateWebProductsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('web_products', function (Blueprint $table) {
            ...
            $table->string('name');
            $table->float('recent_price', 12, 2)->unsigned()->nullable();
            $table->float('previous_price', 12, 2)->unsigned()->nullable();
            ...
            $table->index(['retailer_id', 'slug', 'deleted_at']);
            $table->index('retailer_product_id');
            $table->index(['retailer_id', 'retailer_product_id']);
            $table->index(['retailer_id', 'retailer_product_id', 'deleted_at']);
            $table->index('brand');
